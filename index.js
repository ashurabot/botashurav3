          ...
          case 'ping':
            ...
            break	
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`PREFIXO COM SUCESSO DE ALTERAÇÃO : ${prefix}`)
					break 
				case 'hilih': 
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'QUAL USUÁRIO ?', text, {quoted: mek})
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Pedidos recebidos, abertos ${body.slice(9)}@c.us`, text) 
				break
				case 'leave': 
				if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `sim, sua posição e de administrador já...🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`𝘆𝗮𝗵𝗵 @${mentioned[0].split('@')[0]} sua posição de administrador foi removida...🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `PARABÉNS,POR SE TORNA UM ADMINISTRADOR DE GRUPO...😎 (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabens🥳 @${mentioned[0].split('@')[0]} você se tornou o administrador do grupo. (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('ESCOLHA A PESSOA QUE VC QUER EXPULSAR!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `VC FOI REMOVIDO KSKSSK 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(` @${mentioned[0].split('@')[0]} VC FOI REMOVIDO KAKSKKS🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(mess.only.group)
					teks = `𝗟ISTA DE ADMIN 😎!! *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCESSO ❭ ATIVADO O RECURSO SIMI NESTE GRUPO') 
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCEDIDO ❭ Habilite o recurso nsfw neste grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCEDIDO ❭ Habilite o recurso nsfw neste grupo')
            ...
            break
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨CEDIDO ❭ ATIVANDO OS RECURSOS ESQUERDOS DE BEM-VINDO NESTE GRUPO')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨CEDIDO ❭ Ativando recursos bem-vindo / saiu deste grupo')
					} else {
				...
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(' *ENVIAR FOTOS COM LEGENDA* ')
					}
					break
				default:
